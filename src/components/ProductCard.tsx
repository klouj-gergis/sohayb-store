...
export default function ProductCard({ productData, currency }: { productData: {
  ...
  return (
    <div className="bg-bg h-48 lg:h-[70vh]  min-w-1/3 rounded-lg shadow-shadow p-0 lg:p-5 flex lg:flex-col items-center gap-3">
      ...
      <div className="w-2/3 lg:w-full h-full lg:h-1/3 flex flex-col justify-between  p-4 px-0 pr-3">
        <span className="flex flex-col gap-1">
          <h3 className="text-md lg:text-lg text-dark-accent font-semibold">{productData.name }</h3>
      <p className="text-xs lg:text-sm text-accent">{productData.description}</p>
        </span>
